...
export const seedAll = mutation({
  args: {},
  handler: async (ctx) => {
    // Clear out existing display data to load the fresh dummy data
    ...
    const teamMembers = await ctx.db.query("team").collect();
      for (const m of [
        { nameAr: "أحمد الشمري", nameEn: "Ahmad Al-Shammari", titleAr: "الرئيس التنفيذي والمؤسس", titleEn: "CEO & Founder", bioAr: "مؤسس اريرانج بيكري بخبرة أكثر من 20 عامًا في صناعة المخابز والأغذية الفاخرة", bioEn: "Founder of Arirang Bakery with over 20 years in premium bakery and food industry", photo: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&q=80", visible: true, order: 1 },
    ...
    if (settings.length === 0) {
      await ctx.db.insert("settings", { key: "contact", value: { phone: "+966 11 234 5678", email: "info@arirangbakery.com", addressAr: "حي العليا، شارع الملك فهد، الرياض، المملكة العربية السعودية", addressEn: "Al-Olaya District, King Fahd Road, Riyadh, Saudi Arabia", hours: "7:00 ص - 11:00 م | 7:00 AM - 11:00 PM" } });
      await ctx.db.insert("settings", { key: "social", value: { instagram: "https://instagram.com/arirangbakery", twitter: "https://twitter.com/arirangbakery", facebook: "https://facebook.com/arirangbakery", whatsapp: "https://wa.me/966112345678" } });
      await ctx.db.insert("settings", { key: "auth", value: { username: "admin", password: "arirang2024" } });
    }
